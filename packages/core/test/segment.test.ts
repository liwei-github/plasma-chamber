...
describe('Segment', () => {
  ...
  it('segment1 is added to segment2', () => {
    ...
    const segmentPattern1 = segment1.add(segment2)
    const segmentPattern2 = segment2.add(segment1)
    assert.equal(segmentPattern1.getAmount().toNumber(), 1500000)
    assert.equal(segmentPattern2.getAmount().toNumber(), 1500000)
  })
